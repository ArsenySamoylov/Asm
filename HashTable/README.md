...
Ноутбук фирмы Acer на процессоре Intel Core i5-10300H 2.50GHz и операционная система GNU/Linux 22.04.1-Ubuntu x86_64.

# Теоретическое введение
**Хеш-таблица** - это контейнер, главная идея которого - "разделяй и властвуй". С помощью хеш-функции множество данных разделяются на непересекающиеся подмножества, с которыми быстрее и удобнее работать. 

Ниже приведены эквивалентные определения, которые часто можно найти в литературе.
...
*значение* (или  ) — данные, которые с этим ключом связаны. 
...
## **Первая часть**
Скорость взаимодействия с хеш-таблицей напрямую зависит от её заселенности (среднего количества элементов в контейнерах) и равномерности заполнения. Хеш-таблица считается хорошей, когда в среднем в каждом контейнере 1.5-2 элементов.
...
В качестве элементов выберем английские слова. В хеш-таблице будут храниться только уникальные слова, так как в противном случае на распределение будет так же влиять частота встречи слов.
...
index_t hash3_strlen   (const data* key)
    {
    assert(key);
    return (index_t) strlen( (const char*) key);
    }
```
...
Из полученных данных видно, что лучшими показателями (наименьшей дисперсией) обладают функции `hash7_gnu` и `hash8_crc32`. Для второй части возьмем функцию `hash8_crc32`, так как она показала чуть лучший результат.
...
Например, это может быть загрузка таблицы из памяти. Однако в основном хеш-таблица - это структура данных, которая долго живет в программе. И соответственно загрузка и удаление таблицы занимают заметно меньшее время по сравнению с временем жизни структуры. Поэтому оставим эти оптимизации за пределом нашей работы.
...
Из данной таблицы видно, что большая часть машинных инструкций уходит на вычисление хеша. Поэтому начнем оптимизации именно с этой функции.
...
Так же она имеет набор инструкции `AVX2`, позволяющий использовать вектор максимальной длиной до 32 байт, что более чем достаточно для наших исходных данных.
...
Посмотрим как изменилась статистика Callgrind`a.
...
Оптимизируем функцию сравнения строк.
...
Инлайн функция `cmp_vectors` уже оптимизирована с помощью интринсиков. Поэтому единственный способ оптимизировать эту функцию, это поменять алгоритм поиска элемента внутри списка, например хранить по два значения одновременно. Но данные оптимизации за рамками нашей работы.
