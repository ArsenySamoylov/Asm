...
Каждое *значение* не обязательно должно иметь уникальный *ключ*. Ситуации, когда несколько *значений* имеют одинаковые *ключи*, называют *коллизиями*. Хеш-таблицы делятся на разные типы, по тому как они борются с *коллизиями*. Два основные метода это:
1. метод цепочек
...
Хеш-функция - это один главных факторов, напрямую определяющий распределение элементов по контейнерам. Именно хеш-функция отвечает, в каком контейнере будет хранится элемент. 
