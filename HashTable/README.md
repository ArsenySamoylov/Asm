...
Ассемблерный код для функции написанной на инлайн-ассемлере
```
.LVL1:
#APP
# 28 "./include/HashFunctions.hpp" 1
        ...
        .att_syntax prefix
```
Отсюда видно, что компилятор действительно подставил код из ассемблерной вставки.
...
`hash8_crc32_intrinsics` | 4.24

Из таблицы видно, что даже функция, написанная на интринсиках, заметно быстрее функции, написанной на простом ассемблере. 

Из этого можно сделать вывод, что при оптимизации лучше использовать ассемблерную вставку или интринсики вместо написании функции на чистом ассемблере.
...
Однако ассемблерный оптимизации сильно услужняют разработку, а так же ухудшают читабельность кода и его портативность.

Ассемблерная реализация прошлой функции была достаточна проста. Она сводилоь к тому, чтобы несколь раз использовать встроенную команду crc32.

Но дальнейшая оптимизация кода, переписыванием его на ассемблере, была бы проблематичной и заняла бы много времени. Поэтому дальнейшие оптимизации будут производится без использования ассемблера.
...
#### **таблица 6.**
Версия программы                | Время поиска, секунды     | Прирост относительной прощлой версии 
--------------------------------|---------------------------|-----
без оптимизаций                 | 3.61                      | -
первая оптимизация              | 2.70                      | 1.34
первая и вторая оптимизации     | 2.11                      | 1,28

Посмотри, как теперь выглядит граф вызозов функций программы.
...
Из графа видно, что значительную часть занимает функция CheckList, проверяющая структуру на лист на возможные ошибки. 

Интерсено, что функция FindElementInHashTable занимаент около 1 триллиона инструкций, без учета инструкций, затраченных вызванными ей функцию.
...
Интересно посмотреть на количество инструкций, если отключить все проверки (assert`ы и CheckList).
...
График вызовов с отсутсвием расчета хеша и проверок. Значение в процентах показывает, какую часть функция занимает по отношение к функкции, вызвавшей её.
