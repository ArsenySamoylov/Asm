...
<br /> 
Невооруженным глазом из таблицы видно, что первые три функции имеют плохое распределение:

1. первая функция использует только один контейнер из *227*
2. вторая функция ограничена, так ASCII код букв английского алфавита  может принимать только 52 значения (26 букв и два регистра). Следовательно, используется максимум 52 контейнера из *227*
...
<details>
<summary> Графики распределений функций </summary>
...
### **Что делаем теперь?**
В первой части мы анализировали хеш-таблицу как абстрактную модель, не углубляясь в детали ее реализации. Теперь же займемся исследованием конкретной модели, реализованной для этой исследовательской работы.
...
### **Тестирование**
Создадим тестовую ситуацию, когда мы ищем большое множество элементов в хеш-таблице. Для поиска будем использовать слова из сказки Льюиса Кэрролла "Алиса в Стране чудес". 
...
Теперь, когда мы определились с базовым типом, которым будут представлены наши данные, перейдем к оптимизации функций расчёта хеша и сравнения элементов.
...
#### **таблица 4.** Затраты ресурсов разных реализаций
Название функции         | Количество инструкций, миллион
-------------------------|---------------------------------
...
Однако у ассемблерной реализация есть один недостаток. 
Функция расчета хеша достаточно компакта, поэтому компилятор может подставить ее напрямую в функцию поиска элемента, тем самым уменьшая количество инструкций. Но компилятор не может сделать это для функции, написанной на ассемблре.

Посмотрим, как изменятся показателей функций, если сделать их инлайн версии.
...
Мы же остановимся на выборе функции написанной на инлайн-ассемблере.
...
Довольно много инструкций для такой небольшой функции. Интересно посмотреть на количество инструкций, если отключить все проверки (`assert` и `CheckList`).
...
График вызовов с отсутствием расчета хеша и проверок. Значение в процентах показывает, какую часть функция занимает по отношению к функции, вызвавшей её.
