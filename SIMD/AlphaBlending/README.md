...
### Что такое Альфа-блендинг
Альфа-блендинг - это один из способов совмещения двух изображений, использующий альфа канал (это канал используется для задает прозрачность пикселя)
Для базового типа изображений был выбран BMP 32bpp в силу своей простоты. В этом формате изображение хранится как двумерный массив пикселей, где цвет каждого пикселя кодируется 32 битами (8 битов на каждый канал RGBA, reg green blue alpha)

### Формула расчетов
Для расчетов была выбрана самая простая формула расчета цвета, где значения альфа канала переднего цвета умножается каждый канал цвета переднего пикселя. Аналогично цвет заднего пикселя умножается на 255 - альфа_переднего. Полученные значения складываются и делятся на 256.

### Использование SIMD-инструкций
Базовый тип для хранения цвета одного пикселя - int32. AVX позволяет работать одновременно с 8-мью пикселями

### Примеры кода
Из примеров кода, который можно найти ниже, разу заметно, что код, неписаный на intrinsic-ах заметно объемнее и менее читабельный. Так же, хорошо формулирующийся алгоритм быстро и понятно пишется при помощи стандартных методов, а код при использовании intrinsic-ов заметно дольше и тяжелее. Написание не оптимизированной функции у меня заняло около 20-30 минут (с учетом отладки), в то время как на оптимизированную версию с intrinsic-ами потребовалось несколько часов.
...
Более тщательный анализ кода, оставим пытливому читателю для самостоятельной работы.

## Приложение
### Исходный код написанный на Си 
#### Стандартная функция
```
