...
Самым известным и распространённым решением является IR от [LLVM` a](). 
...
Основной выступает абстрактный класс `Value`, который хранит `name` - имя объекта (так же Value хранит константу - тип конкретного объекта, что, на самом деле является излишним; однако это может пригодиться при отладке программы). От этого абстрактного класса наследуются классы:
...
Класс `Module`, определенный в  `srs/IR/Module.h`, является массивом объектов `Function` и `GlobalVar`. Объект `Module` представляет собой исходную программу на моем языке.
